...
Enny.FlowComponent = function FlowComponent (data) {
    ...
    // Listen
    if (data.event || TYPES(data.type, TYPES.listener)) {
        data.type = TYPES.listener;
    }

    // Stream handler
    if (data.stream || TYPES(data.type, TYPES.streamHandler)) {
        data.type = TYPES.listener;
    ...
    // Data handler
    if (data.data || TYPES(data.type, TYPES.dataHandler)) {
        data.type = TYPES.dataHandler;
        data.handler = data.data || data.method;
    }

    // Error handler
    if (data.error || TYPES(data.type, TYPES.errorHandler)) {
        data.type = TYPES.errorHandler;
        data.handler = data.error || data.method;
    }

    // Link
    if (data.link || TYPES(data.type, TYPES.link)) {
    ...
    // Load
    if (data.load || TYPES(data.type, TYPES.emit)) {
