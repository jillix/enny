...
const TYPES = (function () {
    ...
    action: { sType: "l", type: "action", icon: "&#x26A1", name: "action" },
    event:  { sType: "m", type: "event", icon: "&#xf077", name: "event" },
    method: { sType: "s", type: "method", icon: "&#xf000", name: "call" }
...
Enny.parseFlowComponent = function (_input, instName) {
    ...
    // Load
    if (TYPES(input[0], TYPES.load)) {
        output.type = TYPES.load;
        output.args = input.slice(1);
    // Emit
    } else if ((eP = Enny.parseMethod(input[0], instName)).method === "emit") {
    ...
    if (TYPES(output.method, TYPES.link)) {
