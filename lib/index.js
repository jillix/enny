...
Enny.Handler.prototype.toFlow = function () {
    var res = [];
    res[0] = (this.isStream ? "" : this.isError ? "!" : ":") + (this.to ? this.to + "/" : "") + this.handler;
    if (!this.args.length) {
        return res[0];
    }
...
Enny.FlowComponent.prototype.toFlow = function () {
    ...
    if (TYPES(data.type, TYPES.listener.type)) {
        ...
        return data.event;
    // Link
    } if (TYPES(data.type, TYPES.link.type)) {
    ...
    } if (TYPES(data.type, TYPES.dataHandler.type) || TYPES(data.type, TYPES.streamHandler.type) || TYPES(data.type, TYPES.errorHandler.type)) {
        debugger
        return new Enny.Handler({
            to: data.to
    ...
    // Handle load
    } if (TYPES(data.type, TYPES.load.type)) {
        return new Enny.Handler({
            args: [[data.target]],
